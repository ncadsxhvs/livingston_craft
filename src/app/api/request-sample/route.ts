...
import { createSampleRequestSchema } from "@shared/validation/sales";
import { apiHandler } from "@backend/lib/api-utils";
import { ValidationError } from "@backend/lib/errors";

export const POST = apiHandler(async (request: Request) => {
  const body = await request.json();

  const result = createSampleRequestSchema.safeParse(body);
  if (!result.success) {
  ...
  const { name, email, phone, address, city, state, zip, message, productName } = result.data;
